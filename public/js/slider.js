      ...
      if (settings.arrows) {
        ...
        $arrows.on('click.' + PLUGIN, function(e) {
          ...
          if (this.getAttribute('data-ikslider-dir') === 'next') {
            show(current + 1);
            if(current > 3 && current < slideLen){
                $('.preview-ul').stop().animate({left:-(current+1-4)*163});
            }else{
                $('.preview-ul').stop().animate({left:0});
            }
          } else {
              show(current - 1);
              // console.log('count',slideLen);
              // console.log('index',current+1);
              // console.log(slideLen-(current+1));
              // console.log('jl',(current-4)*163);
              if(current+1 == slideLen ){
                  $('.preview-ul').stop().animate({left:-(slideLen-4)*163});
              }else if(current < 4){
                  $('.preview-ul').stop().animate({left:0});
              }else{
                  $('.preview-ul').stop().animate({left:+(4-(current+1))*163});
              }
